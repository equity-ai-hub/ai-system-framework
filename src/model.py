...
class Model:
    def __init__(self, model_name, dataset_id, df_one_hot, df_preproc, target, sensitive_attr):
    ...
    def train(
        self, n_folds: int = 10, data_dir: str = None, sampling_method: str = "no_sampling", sample_weight: Any = None
    ):
        ...
        valid_values = ["no_sampling", "undersampling", "oversampling"]
        ...
        for train_idx, val_idx in skf.split(X, y):
            ...
            val_fold_preproc = pd.concat([X_preproc_fold_val, y_preproc_fold_val, y_pred, y_pred_proba], axis=1)
            val_fold_oh = pd.concat([X_fold_val, y_fold_val, y_pred], axis=1)

            train_fold_preproc = pd.concat([X_preproc_fold_train, y_preproc_fold_train], axis=1)
            train_fold_oh = pd.concat([X_fold_train, y_fold_train], axis=1)

            # metrics = Metrics.calculate_metrics(y_fold_val, y_fold_pred)
            scores = Metrics.metrics_scores_aif360(df=val_fold_preproc, y_pred=y_fold_pred)
            self.scores = {
            ...
            if data_dir is not None:
                ...
                self._export_training_artifacts(
                    data_dir, train_fold_oh, train_fold_preproc, val_fold_oh, val_fold_preproc, n_folds
                )
